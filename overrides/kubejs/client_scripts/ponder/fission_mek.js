...
Ponder.registry((allthemods) => {
	...
	.scene('fission_mek','Ядерный реактор Mekanism', 'kubejs:fission_mek',
	...
	(scene, util) => {
			...
			scene.text(80, 'Рёбра должны состоять из блоков корпуса', [0, 4.5, 4.5]).placeNearTarget();
			...
			scene.text(80, 'Стены могут состоять из блоков корпуса или стекла', [0, 2.5, 2.5]).placeNearTarget();
			...
			scene.text(60, 'Разместите внутри ядерные тепловыделяющие элементы, чтобы собрать топливные стержни', [2.5, 2.5, 2.5]).placeNearTarget();
			...
			scene.idle(10)
			scene.text(60, 'Поместите контролирующие стержни на вершину каждого топливного стержня', [2.5, 3.5, 2.5]).placeNearTarget();
			scene.idle(20);
			scene.addKeyframe();
			scene.idle(60);
