...
Ponder.registry((allthemods) => {
	...
	.scene('induction_mek','Mekanism: индукционная матрица', 'kubejs:induction_matrix',
	...
	(scene, util) => {
			...
			scene.text(80, 'Индукционная матрица используется для хранения огромного количества энергии.', [0, 4.5, 4.5]).placeNearTarget().attachKeyFrame();
			scene.idle(85);
			...
			scene.text(80, 'Рёбра должны состоять из блоков корпуса', [0, 4.5, 4.5]).placeNearTarget().attachKeyFrame();
			scene.idle(5);
			...
			scene.text(80, 'Грани могут состоять из блоков корпуса или структурного стекла.', [0, 2.5, 2.5]).placeNearTarget().attachKeyFrame();
			...
			scene.text(60, 'Порты используются для передачи энергии.', [1.5, 1.5, 0]).placeNearTarget().attachKeyFrame();
			scene.idle(80);
			
			scene.text(80, 'Режим портов изменяется конфигуратором.', [1.5, 1.5, 0]).placeNearTarget().attachKeyFrame();
			...
			scene.text(60, 'Индукционные ячейки увеличивают запас энергии.', [2.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
			scene.world.setBlock([2, 1, 2], 'mekanism:basic_induction_cell', false);
            scene.world.showSection([2, 1, 2], Facing.down)
            scene.idle(65);
			scene.text(60, 'Индукционные поставщики увеличивают скорость передачи энергии.', [2.5, 2.5, 2.5]).placeNearTarget().attachKeyFrame();
			scene.world.setBlock([2, 2, 2], 'mekanism:basic_induction_provider', false);
            scene.world.showSection([2, 2, 2], Facing.down)
            scene.idle(65)
			
			scene.text(60, 'Матрица должна содержать хотя бы одну ячейку и один поставщик.', [2, 2.5, 2.5]).placeNearTarget().attachKeyFrame();
			scene.idle(65)
