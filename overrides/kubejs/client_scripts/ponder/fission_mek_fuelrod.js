...
Ponder.registry((allthemods) => {
	...
	.scene('fission_mek_fuelrod','Ядерный реактор Mekanism: топливная сборка', 'kubejs:fission_mek',
	...
	(scene, util) => {
			...
			scene.text(80, 'Разместите внутри ядерные тепловыделяющие элементы, чтобы собрать топливные стержни', [2.5, 2.5, 2.5]).placeNearTarget().attachKeyFrame();
			...
			scene.text(120, 'Топливный стержень состоит из нескольких ядерных тепловыделяющих элементов с контролирующими стержнями наверху.', [1.5, 2.5, 2.5]).placeNearTarget();
			scene.idle(40);
			scene.addKeyframe();
			...
			scene.text(80, 'Поместите контролирующие стержни на вершину каждого топливного стержня', [1.5, 3.5, 2.5]).placeNearTarget().attachKeyFrame();
			scene.idle(90);
			
			scene.text(80, 'Блоки контролирующих стержней размещаются на один блок ниже потолка.', [1.5, 3.5, 2.5]).placeNearTarget().attachKeyFrame();
			scene.idle(90);
			
			scene.text(60, 'Топливные стержни не должны соприкасаться', [1.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
			...
			scene.text(80, 'Несколько топливных стержней лучше всего располагать в шахматном порядке.', [1.5, 1.5, 2.5]).placeNearTarget().attachKeyFrame();
			scene.idle(100);
