...
RecipeViewerEvents.addInformation('item', allthemods => {
    allthemods.add('justdirethings:polymorphic_catalyst', [
        '§8Бросьте §cполиморфный катализатор§8 в §1воду§8, чтобы получить полиморфную жидкость'
    ])
    allthemods.add('justdirethings:portal_fluid_catalyst', [
        '§8Бросьте §dкатализатор портальной жидкости§8 в полиморфную жидкость§8 в §dКрае§8, чтобы получить §5нестабильную портальную жидкость'
    ])
})

RecipeViewerEvents.addInformation('fluid', allthemods => {
    allthemods.add("advanced_ae:quantum_infusion_source", [
        '§8В реакционной камере: §e4000 мБ воды§8 + §e1 квантово-наполненная пыль§8 = §b1000 мБ квантовой инфузии'
    ])
    allthemods.add("justdirethings:polymorphic_fluid_source", [
        '§8Бросьте §cполиморфный катализатор§8 в §bводу§8, чтобы получить полиморфную жидкость'
    ])
    allthemods.add("justdirethings:unstable_portal_fluid_source", [
        '§8Бросьте §dкатализатор портальной жидкости§8 в §dполиморфную жидкость§8 в §5Крае§8, чтобы получить §5нестабильную портальную жидкость'
    ])
})
