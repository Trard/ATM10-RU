...
StartupEvents.registry('block', allthemods => {
    allthemods.create('magical_soil').displayName('§bМагическая почва').grassSoundType().mapColor('grass').hardness(0.6);

    allthemods.create('air_essence_block').displayName('Блок эссенции воздуха').stoneSoundType().mapColor('stone').hardness(0.6).renderType('translucent').notSolid();
    allthemods.create('earth_essence_block').displayName('Блок эссенции земли').stoneSoundType().mapColor('stone').hardness(0.6).renderType('translucent').notSolid();
    allthemods.create('fire_essence_block').displayName('Блок эссенции огня').stoneSoundType().mapColor('stone').hardness(0.6).renderType('translucent').notSolid();
    allthemods.create('water_essence_block').displayName('Блок эссенции воды').stoneSoundType().mapColor('stone').hardness(0.6).renderType('translucent').notSolid();

    allthemods.create('blaze_powder_block').displayName('Блок огненного порошка').stoneSoundType().mapColor('stone').hardness(1);
    // allthemods.create('blaze_block').displayName('Blaze Block').stoneSoundType().mapColor('stone').hardness(1);
    allthemods.create('saltpeter_block').displayName('Блок селитры').stoneSoundType().mapColor('stone').hardness(1);
})

StartupEvents.registry('item', allthemods => {
    
    if (Platform.isLoaded("eternal_starlight")) {
        allthemods.create('starlight_prediction').displayName('Обобщённое предсказание Звёздного света');
    }

    allthemods.create('silent_allthemodium_plate').displayName('Тихая всемодиевая пластина').glow(true);
    allthemods.create('silent_vibranium_plate').displayName('Тихая вибраниумная пластина').glow(true);
    allthemods.create('silent_unobtainium_plate').displayName('Тихая унобтаниумная пластина').glow(true);

    allthemods.create('atm_star_shard_1').displayName('Фрагмент звезды ATM')
    allthemods.create('atm_star_shard_2').displayName('Фрагмент звезды ATM')
    allthemods.create('atm_star_shard_3').displayName('Фрагмент звезды ATM')
    allthemods.create('atm_star_shard_4').displayName('Фрагмент звезды ATM')
    allthemods.create('atm_star_shard_5').displayName('Фрагмент звезды ATM')
    ...
    allthemods.create("allthemodium_solar_sail_package").displayName("Комплект всемодиевого солнечного паруса").component("dysoncubeproject:sphere_component_solar_sail", Java.cast("java.lang.Integer", 64))
    allthemods.create("allthemodium_beam_package").displayName("Комплект всемодиевого лучевого модуля").component("dysoncubeproject:sphere_component_beam", Java.cast("java.lang.Integer", 16))
...
global.iceAndFirePearls = (/** @type {$ServerLevel_}} */ level, /** @type {$Player_}} */ player, /** @type {$InteractionHand_}} */ hand) => {
    ...
    if (closest != null || structureResult != null) {
        ...
        player.statusMessage = "Поблизости не найден дракон или драконье логово..."
        // console.log(`IceAndFire Pearl event took: ${currentStopwatch.elapsed("milliseconds")} ms`)
        return false
    }
}
...
StartupEvents.registry('fluid', allthemods => {
    allthemods.create('liquid_souls', 'thin')
        .tint(0xc9eaff)
        .displayName('Жидкие души')
        .noBlock()

    allthemods.create('unrefined_liquid_souls', 'thin')
        .tint(0xccebff)
        .displayName('Неочищенные жидкие души')
        .noBlock()

    allthemods.create('liquid_aureal', 'thin')
        .tint(0x7abaff)
        .displayName('Жидкий Ореал')
        .noBlock()
})
